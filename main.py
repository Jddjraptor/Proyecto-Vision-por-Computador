...
def procesar_rectangulo(frame, x1, y1, x2, y2, tecla, tiempo):
    ...
    contrasted_image = cv2.convertScaleAbs(crop_image, alpha=1.1, beta=50)

    # Aplicar desenfoque Gaussiano
    blur = cv2.GaussianBlur(contrasted_image, (3, 3), 0)
...
while capture.isOpened():
    ...
    # --- RECTÁNGULO 1 (salto) ---
    procesar_rectangulo(frame, 50, 10, 250, 210, "s", 0.15)

    # --- RECTÁNGULO 2 (izquierda) ---
    procesar_rectangulo(frame, 50, 250, 250, 450, "left", 0.3)
    
    # --- RECTÁNGULO 3 (poder) ---
    procesar_rectangulo(frame, 400, 10, 600, 210, "z", 0.05)
    
    # --- RECTÁNGULO 4 (derecha) ---
    procesar_rectangulo(frame, 400, 250, 600, 450, "right", 0.3)
